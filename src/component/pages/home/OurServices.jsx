...
export default function Services() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const cards = res?.data?.data?.services_card;
        setServices(Array.isArray(cards) ? cards : []);
      } catch (err) {
  ...
  const DesktopView = () => {
    const distributeInColumns = (arr, numCols) => {
      const columnCount =
        Number.isInteger(numCols) && numCols > 0 ? numCols : 1;
      const source = Array.isArray(arr) ? arr : [];
      const cols = Array.from({ length: columnCount }, () => []);
      source.forEach((item, i) => cols[i % columnCount].push(item));
      return cols;
    };
