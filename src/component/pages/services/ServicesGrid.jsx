...
const ServicesGrid = () => {
  ...
  // Default icon component to use when service.icon is not defined
  const DefaultIcon = () => <TbPlane size={25} className="text-blue-500" />;
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
          {services.map((service) => {
            ...
            return (
              <div
                key={service.id}
                className="group relative bg-white rounded-2xl shadow-lg overflow-hidden transform transition-all duration-300 hover:shadow-2xl hover:-translate-y-1 h-full cursor-pointer"
                onMouseEnter={() => setHoveredCard(service.id)}
                onMouseLeave={() => setHoveredCard(null)}
                onClick={() => handleCardClick(service)}
              >
                <div className="absolute inset-0 bg-gradient-to-br from-blue-50 to-indigo-50 opacity-5 group-hover:opacity-10 transition-opacity duration-300"></div>
                ...
                <div className="relative p-6 md:p-8 flex flex-col md:flex-row items-start md:items-stretch md:space-x-14 space-y-6 md:space-y-0 h-full">
                  ...
                  <div className="flex-1 min-w-0 md:ml-12 flex flex-col h-full">
                    ...
                    <div className="flex items-center justify-between mt-6 md:mt-8 pt-4 md:pt-6 border-t border-gray-100 md:border-transparent md:pt-0 md:border-0 mt-auto">
                      ...
                      <div className="flex items-center justify-center w-14 h-14 md:w-16 md:h-16 rounded-full bg-gray-100 group-hover:bg-white transition-all duration-300">
                        <IconComponent />
                      </div>
                    </div>
                  </div>
