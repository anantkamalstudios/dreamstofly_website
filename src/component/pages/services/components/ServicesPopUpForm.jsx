...
const ServicesPopUpForm = ({
  ...
  const handleCountryCodeChange = (name, value) => {
    setCountryCodesState((prev) => ({ ...prev, [name]: value }));
  };
      ...
      case "phone":
        return (
          <div className="flex border border-gray-300 rounded-lg overflow-hidden">
            <select
              value={countryCodesState[field.name] || "+91"}
              onChange={(e) =>
                handleCountryCodeChange(field.name, e.target.value)
              }
              className="px-3 border-r bg-gray-50 text-sm"
            >
              {countryCodes.map((country) => (
                <option key={country.code} value={country.code}>
                  {country.flag} {country.code}
  ...
  return (
    <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-xl w-full max-w-lg max-h-[90vh] overflow-hidden relative">
        <button
          onClick={onClose}
